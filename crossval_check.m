%% Проверка качества на кросс-валидации выборки
...
if ~exist('external_input','var')
    ...
    fprintf('(%s) %s\n', datetime('now','Format','HH:mm:ss'), 'Настройка по параметрам файла')
    % Ручная настройка
    clearvars;
    %%% Исходные данные
    dset_folder_name = 'az_scan_30';
    t_select_v = [ 3 ];                     % маска выбора преобразований
   

    % for rm = { 'minalien' 'mhist' }%'nmin' 'buryi' }

    %%% Параметры алгоритма отбора
    reduction_method = 'minalien';          % метод редукции пространства признаков
...
hist_edges = single(linspace(0,hist_max_value,n_hist_edges));    % значения метрик для постр. гистограммы
...
for i_type = 1:n_types
  % Загрузка данных
    ...
    load(in_filename)           % загрузить данные
  % Кроссвалидация  
    fprintf('(%s) %s\n', datetime('now','Format','HH:mm:ss'),[ 'Редукция ' t_type ]) 
    qcheck = QualityCheck(...                       % объект проверки качества
    ...
    qcheck.setup_classifier( ...                    % настройка классификактора
...
if show_graphics    % запуск визуализации
    extern_input = 1; 
    crossval_check_vis; 
...
function [perf_score, rho_min, varargout] = process_cv_results(cv_results)
    ...
    if nargout>=4 
        [~, ind_dim] = max(rhos_max); % индекс лучшей размерности (т.е. лучший случай вообще)
