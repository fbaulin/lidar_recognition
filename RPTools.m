classdef RPTools
    ...
    methods(Access = public, Static = true)
        ...
        %   meta – cell array, содержащий информацию для обновленного массива реализаций ДП
            ...
            signal_power = sum(rps.^2,2)/(dim_rp-1);  % дисперсия огибающей на интервале, соответ. 3м метрам
...
%             nDim = 2*nDim;  % удвоение для уравнивания числа отсчётоы ПФ и ВП
        ...
        % Выполнение ПКЛ. Если на вход передаётся один аргумент, то выполняется синтез матрицы преобразования.
        % Если на вход передаётся два аргумента, то второй используется в качестве матрицы преобразования.
        % Аргументы:
        %   rps - выходное множество
        %   v{1} - матрица преобразования
        % Вывод:
        %   Результаты преобразования[, коэффициенты преобразования]
            if nargin == 2,    coeff = varargin{1};
            else,               coeff = pca(rps);       % матрица преобразования
            end
        ...
        % Это множество векторов дополняется обратными векторами. Вычисляются обратные значения метрик: чем меньше
        % метрика, тем с большим весом она учитывается. Полученное множество загоняется в АГК, в результате чего 
        % формируется преобразовнное множество.
        % Аргументы:
        %   rps - входное множество (ДП)
        %   trgs - целевые векторы
        % Вывод:
        %   Результат преобразования[, коэффициенты преобразования: x*coeff]
            [object_names, ~, ci] = unique(trgs,'rows');    % преобразование имён в индексы
            [~, dim_rps] = size(rps);
            n_obj = size(object_names,1);
            rho_cell = cell(1,(n_obj^2));
            % мектрики между i и j-м объектами. Перебор
            for i_obj = 1:(n_obj-1)
                for j_obj = (i_obj+1):n_obj
                    c1 = rps(ci==i_obj, :);    % формирование первой матрицы
                    c2 = rps(ci==j_obj, :);    % формирование второй матрицы
            ...
            rho_cell = vertcat(rho_cell, -rho_cell);    % векторы и обратные векторы
            weights = sum(rho_cell.^2, 2);        % весовые коэффициенты, равные длине в ^2
            weights(weights==min(weights))=max(weights); % по сути, наименьшие веса исключаются - на случай 
            % чрезмерного влияния близкого к нулю веса или множества нулевых весов (FIXME: HACKY!)
            rho_cell = rho_cell./weights;   % делённый на одну длину - нормированный, на две - обратный учет длин
            coeff = pca(rho_cell);          % матрица преобразования
            varargout{1} = rps*coeff;       % преобразование
            if nargout>1, varargout{2} = coeff; end
        end
        
        % DCA
        function [varargout] = ldca(rps, trgs)
        ...
        % Это множество векторов дополняется обратными векторами. Вычисляются обратные значения метрик: чем меньше
        % метрика, тем с большим весом она учитывается. Идея состоит в том, что не сложные случаи (когда метрика большая
        % должны оказывать меньшее влияние на формирование матрицы преобразования. Большое влияние малых по величине
        % (шумовых) метрик будет компенсироваться изотропностью их распределения. Полученное множество загоняется 
        % в АГК, в результате чего формируется преобразовнное множество.
        % Аргументы:
        %   rps - входное множество (ДП)
        %   trgs - целевые векторы
        % Вывод:
        %   Результат преобразования[, коэффициенты преобразования: x*coeff]
            ...
            weights = sum(minrho.^2, 2);        % весовые коэффициенты, равные длине в ^2
            weights(weights==min(weights))=max(weights); % по сути, наименьшие веса исключаются - на случай 
            % чрезмерного влияния близкого к нулю веса или множества нулевых весов (FIXME: HACKY!)
            minrho = minrho./weights;   % делённый на одну длину - нормированный, на две - обратный учет длин
